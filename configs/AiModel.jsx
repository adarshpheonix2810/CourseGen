...
const RETRY_CONFIG = {
  ...
  initialDelay: 3000,             // Initial delay in ms (3 seconds)
  maxDelay: 60000,                // Maximum delay in ms (1 minute)
  backoffFactor: 3,               // How aggressively to increase delay (multiplier)
  jitter: true,                   // Whether to add random jitter
  statusCodesToRetry: [429, 500, 502, 503, 504], // HTTP status codes to retry
  retryableErrors: [              // Error messages that indicate a retry is possible
    'overloaded',
    'service unavailable',
    'rate limit',
    'too many requests',
...
// Add retry logic for API calls with exponential backoff and jitter
const withRetry = async (fn, customConfig = {}) => {
  const config = { ...RETRY_CONFIG, ...customConfig };
  let attempts = 0;
  let delay = config.initialDelay;
  
  // Create a more readable error message
  const createError = (error, isFinal = false) => {
    const status = error.status || error.statusCode || 'N/A';
    const message = error.message || 'Unknown error';
    const retryInfo = isFinal ? ' (no more retries)' : ` (attempt ${attempts}/${config.maxRetries})`;
    return new Error(`API Error [${status}]${retryInfo}: ${message}`);
  };
  ...
  const isRetryableError = (error) => {
    ...
    return config.statusCodesToRetry.includes(status) || 
           config.retryableErrors.some(term => message.includes(term));
  };
  
  // Calculate delay with jitter
  const calculateDelay = (baseDelay) => {
    if (!config.jitter) return baseDelay;
    const jitter = Math.random() * 1000; // Up to 1s jitter
    return Math.min(Math.floor(baseDelay + jitter), config.maxDelay);
  };
  ...
  while (attempts < config.maxRetries) {
    ...
    try {
      ...
      // If we can't retry or this was our last attempt, throw the error
      if (!shouldRetry || isLastAttempt) {
        console.error(`Final attempt (${attempts}/${config.maxRetries}) failed:`, error.message);
        throw createError(error, true);
      }
      
      // Calculate next delay with exponential backoff and jitter
      const nextDelay = calculateDelay(delay);
      console.warn(`Attempt ${attempts}/${config.maxRetries} failed (${error.message}). ` +
      ...
      sendMessage: async (prompt) => {
        return withRetry(async () => {
          try {
            ...
            console.error("Error in GenerateCourseLayout_AI:", error);
      ...
      sendMessage: async (prompt) => {
        return withRetry(async () => {
          try {
            ...
            console.error("Error in GenerateChapterContent_AI:", error);
